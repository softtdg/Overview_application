...
class _QAOutEditEntryState extends State<QAOutEditEntry> {
  ...
  Future<void> GetQAOutSOPById() async {
    ...
    try {
      ...
      if (mounted) {
        AppToast.error(context, 'Failed to load QA Out entry');
      }
    } finally {
      setState(() {
        isLoading = false;
  ...
  Future<void> HandleUpdateQAOutEntry() async {
    try {
      ...
      AppToast.success(context, "QA Out entry updated successfully");
      Navigator.pop(context, true);
    } catch (e) {
      print("Error updating QA Out entry: $e");
      if (mounted) {
        AppToast.error(context, 'Failed to update QA Out entry');
      }
    } finally {
      setState(() {
        isLoading = false;
